...
const PCT_RANGES = ["0%", "De 1% a 25%", "De 26% a 50%", "De 51% a 75%", "De 76% a 99%", "100%", "Sem mecanismos disponíveis"];
const DAYS_TO_FIRST = [
  "Até 7 dias",
  "De 8 a 15 dias",
  "De 16 a 30 dias",
...
function pctBand(value, available) {
  if (!available) return "Sem mecanismos disponíveis";
  if (value == null) return "Sem mecanismos disponíveis";
  if (value <= 0) return "0%";
...
function buildPayload(clients, cmRows, mechanisms, cancellations = []) {
  ...
  const { rows: deduped, duplicatePairs } = dedupeClientMechanisms(cmRows);
  if (duplicatePairs > 0) {
  ...
  return {
    ...
    summary: {
      ...
      distinctMechanismTypes: new Set(deduped.map((r) => r.mecanismo_id)).size,
      catalogMechanisms: mechanisms.length,
