...
export const portalMetricCatalog = {
  ...
  top_meeting_types: {
    ...
    description:
      "Distribuição exclusiva do CSV de tipos de reunião (gráfico Reuniões por tipo). Não altera KPIs operacionais.",
    aggregation: "top",
    unit: "meetings",
    executor: "meetings",
    summaryField: null,
  ...
  average_days_to_first_mechanism: {
    ...
    unit: "days",
    formula:
      "Na coorte com os quatro prazos válidos: dias = primeira client_meetings.start_time cujo event_name contém Ativação das Engrenagens ou Implementação − data inicial. Calcular a mediana.",
    numerator: null,
    denominator: "clientes da coorte única dos quatro prazos",
    dateStart: "clients.data_inicio_ciclo (fallback created_at)",
    dateEnd: "primeira client_meetings.start_time de Ativação das Engrenagens ou Implementação",
    inclusionRules: ["event_name contém Ativação das Engrenagens ou Implementação", "cliente presente na coorte única"],
    exclusionRules: ["sem data inicial", "sem reunião qualificada", "diferença negativa", "ordem temporal inconsistente"],
    supportedFilters: ["engineer", "status", "onboardingStatus"],
    sources: [
      { schema: SUPABASE, table: "clients", column: "data_inicio_ciclo" },
      { schema: SUPABASE, table: "client_meetings", column: "start_time" },
